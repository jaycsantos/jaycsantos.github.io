...
import { ProjectItem, ProjectContext, ProjectProps } from '../components/ProjectItem'
...
export default function Timeline() {
  ...
  return (
    <ProjectContext.Provider value={{ project, setProject }}>
      <h2 className="hidden mb-4 text-xl font-medium print:block">Experience / Notable Projects<hr /></h2>
      <ul className="flex-grow-0 bg-fixed bg-center bg-no-repeat transition-all duration-500 delay-200 timeline-grid">
        {coloredData.map((item, index) => (
          <React.Fragment key={'d' + index}>
            <li key={'item' + index}
              className="relative pb-4 bg-clip-border sm:pr-8 sm:border-r print:px-0"
              style={{
                '--border-start': item.color ?? '#808080',
                '--border-end': coloredData[index + 1]?.color ?? '#808080'
              }}
            >
              <TimelineItem key={item.id} item={item} className="sticky top-24 pb-4 print:static print:pt-0 print:pb-2 print:pr-4" />
            </li>
            <li className="flex relative flex-col gap-2 pb-4 pl-4 sm:px-8 print:px-4">
              {Object.keys(item.projects ?? {}).toReversed().map((year) => {
                return (
                  <ul key={year} className="grid grid-cols-1 gap-2 md:grid-cols-2 print:flex print:flex-col print:gap-3 print:list-disc" >
